<template>
  <div>
    <div class="container">
      <div v-for="tileRow in tiles" :key="tileRow[0].id[0]" class="tile-row">
        <div
          v-for="tile in tileRow"
          :key="tile.id"
          :id="tile.id"
          class="tile"
          :class="{ path: tile.path }"
...
const tiles = ref<Tile[][]>(
  [...Array(10).keys()].map((i) => {
    return [...Array(10).keys()].map((j) => {
      return { id: `${i}${j}`, value: ".", path: false };
    });
  })
);
